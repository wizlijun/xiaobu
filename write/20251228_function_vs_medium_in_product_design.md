...
## "Function" vs. "Medium"：不同的产品哲学

在设计产品时，特别是作为创业者，有一个根本性的战略选择：构建狭窄的解决方案（function）还是开放的平台（medium）。

- **功能型产品**像传统应用或功能——设计用于极好地完成一组任务。它通常有明确的、开发者定义的目标（解决特定问题）并引导用户完成该工作流。
- **媒介型产品**（或平台）设计得更通用和开放，允许用户追求自己的目标并在平台之上定义新的用例。
...
每种方法都有优缺点：
- **专注的应用**可以开箱即用提供精致、直接的体验（无需特殊技能即可获得价值），但在适应性和用户投入方面可能触及天花板
- **类媒介的平台**最初更难设计和做对——它必须既开放又易于接近，并依赖用户共同创造价值。但当它成功时，可以产生更大的影响：它利用许多用户的创造力，可以满足多样化的需求，并经常成为人们或组织运作方式的一部分（这使它非常有价值且难以取代）
